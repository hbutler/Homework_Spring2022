...
```
 ...
 Because each group accounts for 1 degree of freedom, the MSE values will be the same: $MS(\text{straw} \mid 0) = SS(\text{straw}\mid 0)/1$, $MS(\text{wood} \mid \text{straw}) = SS(\text{wood}\mid \text{straw})/1$, $MS(\text{brick} \mid \text{straw, wood}) = SS(\text{brick}\mid \text{straw, wood})/1$.
 
```{r, echo = FALSE}
tibble("Estimate" = c(bh, NA)
      , "Degrees of Freedom" = c(1, 1, 1, 9-3) 
      , "Sum of Squares" = c(SS_X1_0, SS_X2_X1, SS_X3_X2, RSS)
      ) %>%
  mutate("Mean Squares" = `Sum of Squares`/`Degrees of Freedom`) %>%
  kable()
```
