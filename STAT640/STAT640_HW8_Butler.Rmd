...
**Answer:** 
Since the design matrix $\bmX_{int}$ will no longer be one of full rank, we must use a generalized inverse to compute $\hat{\bbh}$. This can be found by partitioning $\bmX_{int} = \begin{bmatrix} \bm1 & \bmX \end{bmatrix}$ and finding the generalized inverse as $\bmG = \begin{bmatrix} \bm0 & \bm0 \\ \bm0 & (\bmX^T\bmX)^{-1} \end{bmatrix}$
