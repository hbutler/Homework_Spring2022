...
$\dfrac{\bmY^\mT(\bmP_4 - \bmP_1)\bmY/ \rank(\bmP_4 - \bmP_1)}{\bmY^\mT(\bmI - \bmP_7)\bmY/\rank(\bmI - \bmP_7)}$
  ...
  1. This is a valid $F$ statistic, according to the three conditions listed above. Since $(\bmP_4 - \bmP_1)$ **is** a projection matrix, we can use Proposition 3.18 to confirm that it is distributed as a $\chi^2$ random vector. Since $\bmP_7$ projects onto the full column space of $\bmX$, the centrality parameter for $\frac{1}{\sigma^2}\bmY^T(\bmI-\bmP_7)\bmY$ will be zero. Finally, because $\bmP_4$ projects onto a subspace of $\bmP_7$, the numerator and denominator will be independent (not sure why??)
  
  2. $H_0: \beta_2 = 0$, $H_a: \beta_2 \neq 0$.

$\dfrac{\bmY^\mT(\bmP_7 - \bmP_5)\bmY/ \rank(\bmP_7 - \bmP_5)}{\bmY^\mT(\bmI - \bmP_7)\bmY/\rank(\bmI - \bmP_7)}$

  1. Using the same justifications as we did for b. above, we can see that this is a valid $F$ statistic.
...
$\dfrac{\bmY^\mT(\bmP_3 - \bmP_2)\bmY/ \rank(\bmP_3 - \bmP_2)}{\bmY^\mT(\bmI - \bmP_6)\bmY/\rank(\bmI - \bmP_6)}$
  ...
  1. No, this is not a valid $F$ statistic, since $\bmP_8$ does not project onto ${\cal C}(\bmX)$. (Although I do think it projects onto a subspace of ${\cal C}(\bmX)$, where the first three components are 0 and the last 6 components of the elements are the same).
...
$\dfrac{\bmY^\mT(\bmP_6 - \bmP_8)\bmY/ \rank(\bmP_6 - \bmP_8)}{\bmY^\mT(\bmI - \bmP_7)\bmY/\rank(\bmI - \bmP_7)}$
  ...
  3. This is not a GLRT, since we are not utilizing all of the information available to test the hypothesis.
  
$\dfrac{\bmY^\mT(\bmP_7 - \bmP_8)\bmY/ \rank(\bmP_7 - \bmP_8)}{\bmY^\mT(\bmI - \bmP_7)\bmY/\rank(\bmI - \bmP_7)}$

  1. This is also a valid $F$ statistic.
